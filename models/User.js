...
User.prototype.validate = function () {
    return new Promise(async (resolve, reject) => {
        ...
        // Only if the username is valid then check to see if its already taken
        ...
        // Only if the email is valid then check to see if its already taken
        if (validator.isEmail(this.data.email)) {
...
User.prototype.register = function () {
    return new Promise(async (resolve, reject) => {
        ...
        this.cleanUp()
        await this.validate()
        // Step #2 only if no validation errors then store data into the database
...
User.prototype.getAvatar = function () {
    this.avatar = `https://en.gravatar.com/avatar/${md5(this.data.email)}?s=128`
}
